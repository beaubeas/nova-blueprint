# This App Python API (backend)

Flask-based backend for This App: market data, indicators, AI analysis, backtesting, and a strategy runtime with multi-user support.
...
```bash
# Database
POSTGRES_USER=jesusban
POSTGRES_PASSWORD=your_secure_password
POSTGRES_DB=jesusban
...
CREATE DATABASE jesusban;
CREATE USER jesusban WITH ENCRYPTED PASSWORD 'your_password';
GRANT ALL PRIVILEGES ON DATABASE jesusban TO jesusban;
\q

# Initialize schema
psql -U jesusban -d jesusban -f migrations/init.sql
```
...
```bash
# Required
DATABASE_URL=postgresql://jesusban:your_password@localhost:5432/jesusban
...
- Proxy config: `jesusban_vue/vue.config.js`
