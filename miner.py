...
def _cpu_random_candidates_with_similarity(
    ...
    try:
        ...
        random_df = random_df[random_df["tanimoto_similarity"] >= SIMILARITY_THRESHOLD]
        if random_df.empty:
            return pd.DataFrame(columns=["name", "smiles", "InChIKey", "tanimoto_similarity"])
        return random_df[["name", "smiles", "InChIKey"]]
    except Exception as e:
    ...
    with ProcessPoolExecutor(max_workers=1) as cpu_executor:
        while time.time() - start < 1800:
            ...
            if not seed_df.empty:
                data = pd.concat([data, seed_df], ignore_index=True, sort=False)
            ...
            if not top_pool.empty:
                cpu_future = cpu_executor.submit(
                    ...
                    1000,
                    config,
                    top_pool.head(3)[["name", "smiles", "InChIKey"]],
                    seen_inchikeys,
                )
