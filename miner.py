...
def main(config: dict):
    ...
    last_top3_molecules = set()
    bt.logging.info("[V4] Advanced tracking initialized: Synergy Matrix")
    ...
    with ProcessPoolExecutor(max_workers=3) as cpu_executor:
        while time.time() - start < 1800:
            ...
            if no_improvement_counter >= 5:
                bt.logging.info(f"[V5-RESTART] Triggering diversity restart (stuck for {no_improvement_counter} iterations)")
                elite_memory = top_pool.head(20).copy()
                bt.logging.info(f"[V5-RESTART] Saved top 20 elites (avg: {elite_memory['score'].mean():.4f})")
                restart_candidates = []
                ...
                if use_synthon_search and synthon_lib is not None:
                    try:
                        pop_A_df = generate_molecules_from_synthon_library(
                            synthon_lib,
                            top_pool.head(10),
                            n_samples=int(n_samples * 0.35),  # 35% tight exploitation
                            min_similarity=0.75,
                            n_per_base=25
                        )
                        if not pop_A_df.empty:
                            ...
                            bt.logging.info(f"[V5-RESTART] Pop A (tight): {len(pop_A_df)} candidates")
                    except Exception as e:
                        bt.logging.warning(f"[V5-RESTART] Pop A failed: {e}")
                ...
                if use_synthon_search and synthon_lib is not None and len(top_pool) >= 60:
                    try:
                        ...
                        pop_B_df = generate_molecules_from_synthon_library(
                            synthon_lib,
                            pop_B_seeds,
                            n_samples=int(n_samples * 0.25),  # 25% medium exploration
                            min_similarity=0.50,
                            n_per_base=15
                        )
                        if not pop_B_df.empty:
                            ...
                            bt.logging.info(f"[V5-RESTART] Pop B (medium): {len(pop_B_df)} candidates")
                    except Exception as e:
                        bt.logging.warning(f"[V5-RESTART] Pop B failed: {e}")
                
                try:
                    pop_C_df = generate_valid_random_molecules_batch(
                        rxn_id,
                        n_samples=int(n_samples * 1.5),  # 150% budget for pure exploration
                        db_path=DB_PATH,
                    ...
                    if not pop_C_df.empty:
                        ...
                        bt.logging.info(f"[V5-RESTART] Pop C (random): {len(pop_C_df)} candidates")
                except Exception as e:
                    bt.logging.warning(f"[V5-RESTART] Pop C failed: {e}")
                
                if restart_candidates:
                    data = pd.concat(restart_candidates, ignore_index=True)
                    data = data.drop_duplicates(subset=["name"], keep="first")
                    bt.logging.info(f"[V5-RESTART] Total restart candidates: {len(data)}")
                else:
                    bt.logging.warning(f"[V5-RESTART] All populations failed, using fallback")
            ...
            # V5: ALWAYS USE QUALITY PRE-RANKING - Not just for excess candidates!
            if iteration > 3 and component_weights and len(data) > 0:
                bt.logging.info(f"[V5-QUALITY] Pre-ranking {len(data)} candidates with synergy-based quality scoring")
                try:
                    original_count = len(data)
                    data = data.copy()  # Important: work with a copy to avoid DataFrame issues
                    ...
                    if len(data) > n_samples * 1.2:
                        ...
                        bt.logging.info(f"[V5-QUALITY] Filtered to {len(data)} candidates ({top_n} top + {len(data)-top_n} random)")
                    
                    data = data.drop(columns=['quality'])  # Drop quality column
                    data = data.reset_index(drop=True)  # Reset index
                    
                    if len(data) < original_count:
                        bt.logging.info(f"[V5-QUALITY] Filtered {original_count - len(data)} low-quality candidates")
                except Exception as e:
                    bt.logging.warning(f"[V5-QUALITY] Quality pre-ranking failed: {e}, continuing without filtering")
            ...
            if False and iteration > 3 and use_synthon_search and synthon_lib is not None:
                current_top3 = set(top_pool.head(3)['name'].tolist())
                new_discoveries = current_top3 - last_top3_molecules
                
                if new_discoveries:
                    bt.logging.info(f"[V4-BURST] New top-3 molecule found! Triggering intensive local search")
                    for mol_name in new_discoveries:
                        try:
                            ...
                            # V4 FIX: Validate burst_df to add smiles and InChIKey columns
                            if not burst_df.empty:
                                burst_df = validate_molecules(burst_df, config)
                            if not burst_df.empty:
                                seed_df = pd.concat([seed_df, burst_df], ignore_index=True) if not seed_df.empty else burst_df
                                bt.logging.info(f"[V4-BURST] Added {len(burst_df)} burst candidates for {mol_name}")
                        except Exception as e:
                            bt.logging.warning(f"[V4-BURST] Failed for {mol_name}: {e}")
                
                last_top3_molecules = current_top3
            
            # V4: CLUSTER EXPLOITATION - DISABLED (was polluting pool with mediocre candidates)
            if False and iteration > 5 and len(top_pool) >= 30 and use_synthon_search and synthon_lib is not None and score_improvement_rate < 0.01:
                try:
                    clusters = cluster_molecules(top_pool.head(50), n_clusters=min(5, len(top_pool)//10))
                    bt.logging.info(f"[V4-CLUSTER] Identified {len(clusters)} molecular clusters")
                    
                    for cluster_id, cluster_df in clusters.items():
                        if len(cluster_df) >= 3:
                            cluster_budget = max(30, int(n_samples * 0.10))
                            cluster_seeds = cluster_df.head(min(8, len(cluster_df)))
                            cluster_mols = generate_molecules_from_synthon_library(
                                synthon_lib, cluster_seeds, cluster_budget,
                                min_similarity=0.72, n_per_base=18
                            )
                            # V4 FIX: Validate cluster_mols to add smiles and InChIKey columns
                            if not cluster_mols.empty:
                                cluster_mols = validate_molecules(cluster_mols, config)
                            if not cluster_mols.empty:
                                seed_df = pd.concat([seed_df, cluster_mols], ignore_index=True) if not seed_df.empty else cluster_mols
                                bt.logging.info(f"[V4-CLUSTER] Cluster {cluster_id}: {len(cluster_mols)} candidates")
                except Exception as e:
                    bt.logging.warning(f"[V4-CLUSTER] Clustering failed: {e}")
